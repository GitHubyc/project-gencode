...
import com.xakj.util.IJdbcTemplate;

import com.xakj.model.AssemblingSqlEntity;
...
public class ${modelName}DaoImpl implements ${modelName}Dao {
	...
	public void savaBatch(final List<${modelName}Entity> ${modelName?uncap_first}List) {
		...
		jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
			public void setValues(PreparedStatement ps, int i) throws SQLException {
				...
				ps.setDouble(${key_index+1}, ${modelName?uncap_first}Entity.get${model_column[key].ChangeColumnName?cap_first}());
				<#--日期类型-->
	...
	public void deleteBatch(final List<${modelName}Entity> ${modelName?uncap_first}List) {
		...
		this.jdbcTemplate.batchUpdate(sql,new BatchPreparedStatementSetter() {
			public void setValues(PreparedStatement ps, int i) throws SQLException {
				...
				<#--浮点类型-->
				<#elseif table_key_type?default('NVARCHAR2') == "DOUBLE">
				ps.setd=Double(1, ${modelName?uncap_first}Entity.get${table_key_change?cap_first}());
				<#else>
	...
	public ${modelName}Entity findOne(${tablekeytype} ${table_key?uncap_first}) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT * FROM ${table_name} WHERE ${table_key?upper_case}=?");

		RowMapper<${modelName}Entity> rowMapper = new BeanPropertyRowMapper<${modelName}Entity>(${modelName}Entity.class);
		return this.jdbcTemplate.queryForObject(sql.toString(), rowMapper, ${table_key?uncap_first});
	}
	...
	@Override
	public List<${modelName}Entity> findAll(Integer currentPage, Integer pageSize, ${modelName}Entity ${modelName?uncap_first}Entity) {
		StringBuffer sql = new StringBuffer();
		List<Object> params = new ArrayList<Object>();
		...
		// ${table_annotation}检索条件拼装
		AssemblingSqlEntity assemblingSqlEntity = ${modelName}AssemblingSql(
				${modelName?uncap_first}Entity, sql, params);
	 ...
	 */
	 @Override
	public boolean exists(${modelName}Entity ${modelName?uncap_first}Entity) {
		if (count(${modelName?uncap_first}Entity) > 0) {
			return true;
		} else {
			return false;
		}
	...
	/**
	 * @param ${modelName?uncap_first}Entity
	 * @param sql
	 * @param params
	 * @return
	 */
	public AssemblingSqlEntity ${modelName?uncap_first}AssemblingSql(
			${modelName}Entity ${modelName?uncap_first}Entity, StringBuffer sql,
			List<Object> params) {
		<#list model_column?keys as key>
		if (!StringUtils.isEmpty(${modelName?uncap_first}Entity.get${model_column[key].ChangeColumnName?cap_first}())) {
			sql.append(" AND ${table_name}.${key?upper_case}=? ");
			params.add(${modelName?uncap_first}Entity.get${model_column[key].ChangeColumnName?cap_first}());
		}
		</#list>
