...
//		return connection;
	...
	public void generate(String tableName, String modelName, String tableAnnotation, String author) throws Exception {
		...
		try {
			...
			// 生成数据字典文件
			generateDataDictionaryFile(databaseMetaData, table_key, table_key_change, table_key_type, tableName, modelName, tableAnnotation, author);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}

	private void generateDataDictionaryFile(DatabaseMetaData databaseMetaData, String table_key, String table_key_change, String table_key_type, String tableName,
			String modelName, String tableAnnotation, String author) throws Exception {
		mkdirs(new File(Constant.DATADICTIONARY_PATH));
		String hasDateType = "false";
		final String path = Constant.DATADICTIONARY_PATH + modelName + "DataDictionary" + ".xml";
		File createPath = new File(path);
		if (!createPath.exists()) {// 避免生成覆盖
			final String templateName = "DataDictionary.ftl";
	...
	public Map<String, Object> resultSetToMap(ResultSet resultSet) throws Exception {
		...
		while (resultSet.next()) {
			...
			object.put("table_key_change", resultSet.getString("COLUMN_NAME").toUpperCase());
			object.put("ColumnSize", resultSet.getString("COLUMN_SIZE"));
			// 字段在数据库的注释
